	...
	if(isset($serverRequest) && ($data_user['serverRequest'] == 'default')) {
		...
		//СТУДЕНТ
		   ...
		   case "1":
		   	...
		   	$query=$conn->prepare("select cast(FFIO AS VARCHAR2(512)) as FFIO, cast(GRUP AS VARCHAR2(512)) as GRUP, cast(FPERSONS AS VARCHAR2(100)) as FPERSONS
		   	...
		   	if($query->execute(array('FFIO' => $data_user['FIO']))) {
  			    ...
  			    $data_user['PERSONS'] = sha1($data_stud_arr[0]['FPERSONS']);
	   ...
	   $_SESSION['persons'] = $data_user['PERSONS'];
