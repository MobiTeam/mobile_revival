<?php
  ...
  require('database_connect_PDO.php');
	...
	$nrec_speciality_array=array();

	// Nrec специальности
	
	$nrec_speciality_array = explode(",", $_GET['nrecs']);

  // $nrec_speciality_array = array('800100000000030A', '8001000000000305','80010000000002DB');
  $abiturient_information = array();
	for ($i=0; $i < count($nrec_speciality_array); $i++) { 
...
order by --NVL(O.fwMark,0) desc, NVL(T.Docs,0) desc, UA.fsFio
	...
	$query->execute(array('fNrec_Plan' => $nrec_speciality_array[$i]));


  $query_bud = $conn->prepare("select C.fName a0, fwFormEd, C.fCode a2, to_char(sysdate,'YYYY') a3,  
                                     sum( decode(sp.fcode,'РФ',UPFS.fNumStud,'Целевой',UPFS.fNumStud,'РФ Крым',UPFS.fNumStud,'РФ (л)',UPFS.fNumStud,0) ) RF, 
                                     sum( decode(SP.fCode, 'Целевой',UPFS.fNumStud, 0) ) Cel, 
                                     sum( decode(sp.fcode, 'ХМАО-'||to_char(to_date(to_char(sysdate,'YYYY'),'YYYY'),'yy'),UPFS.fNumStud, 'Целевой ХМАО',UPFS.fNumStud, 0) ) HMAO,
                                     sum( decode(sp.fcode, 'Целевой ХМАО',UPFS.fNumStud, 0) ) HMAO_Cel,
                                     sum( decode(SP.fCode, 'ВБ',UPFS.fNumStud, 0) ) VB, Q.fcode kv
                                from U_Plan UP inner join Catalogs            C on   UP.fcSpeciality =  C.fNrec 
                                       inner join U_Plan_FinSource UPFS on UPFS.fcU_Plan     = UP.fNrec
                                       inner join Spkau              SP on UPFS.fcFinSource  = SP.fNrec
                                       inner join Catalogs            Q on UP.fcqualification= Q.fnrec
                       where UP.fNrec = :fNrec_Plan
                      group by C.fName, fwFormEd, C.fCode, to_char(sysdate,'YYYY'), Q.fcode");

  $query_bud->execute(array('fNrec_Plan' => $nrec_speciality_array[$i]));
  $row_bud = $query_bud->fetch();

  $bud_rf = $row_bud['RF'] *1.0 ;
  $bud_hm = $row_bud['HMAO'] *1.0 ;
  $bud_vb = $row_bud['VB'] *1.0 ;
  $bud_sum = $bud_rf+$bud_hm+$bud_vb;

	$count = 0; 

  while ($row = $query->fetch()) {
		$abiturient_information[$row['PLAN_NAME']][$count] = array(
												'fio'	 => $row['FSFIO'],
												// 'rownum' 	=> $row['rownum'],
												'pr'	 => $row['FWPRIORITY'],
												// 'lgota'		=> $row['LGOTA'],
...
echo json_encode($abiturient_information, JSON_UNESCAPED_UNICODE);
