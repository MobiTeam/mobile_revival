...
function showStudAppoint(){
	...
	for(var i = 0; i < appoint.length; i++){
		...
		htmlCode += "<div style='clear:both'></div><div class='rate_footer'>Дата зачисления: " + appoint[i].date_zach + "<br> Дата окончания: " + appoint[i].date_konch + "</div>";
		htmlCode += "</div>";
	}
...
var getAuthInfo = function(infoObj){
	...
	if (authObj.FIO != "undefined"){
		...
		saveValue('email', authObj.email);
		saveValue('phone', authObj.phone);
...
function setUserSettings() {
	...
	$('.user_mail').val(getValue("email"));

	$('.user_phone').val(getValue("phone"));
...
function createHtmlSettings() {
	...
	htmlSettings += '<div class="settings_box_inputs"><div class="settings_box_inputs_item">email<br> <input class="user_mail" value="" type="text" placeholder="Введите email" /></div></div>';
	htmlSettings += '<div class="settings_box_inputs"><div class="settings_box_inputs_item">Контактный телефон<br> <input class="user_phone" value="" type="text" placeholder="Введите контактный телефон" /></div></div>';
...
function createHTMLAbitBlock(data) {

	var resStr =  '<div class="person_contacts_info contr_shadow" style="max-width: 700px; margin:0 auto; padding-bottom:10px;">\
					<div class="abit_pers_fio_box">\
					<div class="person_contacts_img"></div>\
					<div class="person_contacts_middle" style="padding: 11px 1%;">\
						<b>' + data.FIO + '</b>\
						<br>\
						<b class="grey_bold_text">форма обучения: ' + (data.FORM_ED == "0" ? "очная" : "заочная") + '</b></div><div style="clear:both"></div></div>';

	var lastItem = data['PRIOR' + 3] ? '3' : data['PRIOR' + 2] ? 2 : 1;
	var nrecs = [];

	for(var i = 1; i <= 3; i++) {
		if(data['PRIOR' + i]) {
			resStr += "<div class='priority_item " + (i == lastItem ? 'last_item' : '') + " '><b>" + i + " приоритет:</b> " + data['PRIOR' + i] + "</div>";
			nrecs.push(data['FCPLAN_PRIOR' + i]);
		}
	}

	resStr += "<button class='abit_detail_btn contr_shadow' onclick='loadDetailsBySpecialities(\"" + nrecs + "\")'>Подробнее | Обновить</button>";
...
function loadDetailsBySpecialities(nrecs) {

	myajax(true, "GET", "oracle/database_abiturient_information.php", { nrecs: nrecs }, false, showAbitDetails, true);
...
function showAbitDetails(details) {
	...
	for(var spec in details) {

		var abitInfoBlock = '<div class="abit_details_results person_contacts_info contr_shadow">';

		abitInfoBlock += "<div class='person_contacts_header abit_spec_header' onclick='rotateBtn(" + ind + ")'>\
								<b>" + spec + "</b><br>\
								<b class='grey_bold_text'>Всего подано заявлений: " + details[spec].length + "</b>\
								<div class='button_arrow down_arrow'><i class='btn_down" + ind + "'></i></div>\
						   </div>";

		abitInfoBlock += "<div style='display:none;' class='det_info" + ind + "'>";

		abitInfoBlock += "<table class='abit_det_info_tbl'>";
		abitInfoBlock += "<tr class='info_tr'><td>№</td><td>ФИО</td><td>Ориг. диплома</td></tr>";

		for(var j = 0; j < details[spec].length; j++) {

			var cp = details[spec][j];

			abitInfoBlock += "<tr class='data_td_abit'>\
								<td>" + (j + 1) + "</td>\
								<td>" + cp.fio + "</td>\
								<td></td>\
							  </tr>";
		...
		abitInfoBlock += "</div>";
		abitInfoBlock += '</div>';
		$('.abit_results').append(abitInfoBlock);
		ind++;

	}

	$('.abit_details_results').fadeIn(150);

}

function rotateBtn(numBl) {
